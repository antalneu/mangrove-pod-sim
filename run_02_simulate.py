...
# Load source: the YEAR-ONE seedling root system. Real Rhizophora growth stages
# put the prop-root cage at 3-5 years, so it cannot be what loads a pod over a
# 12-month window - in year one the plant has a primary root and little else.
# proproots.grow_prop_roots stays available for multi-year windows.
...
print(f"Year-one root system: {len(roots.nodes)} nodes.")
...
viz.render_root_system(pod, roots, "outputs/02_roots.png",
                       title="year-one root system in pod")
